...
## [1.0.0] - 2025-10-07
### ➕ Agregado
- Creada la ruta `/reenviar-confirmacion` 
- Creada la funcion `reenviarConfirmacion` dentro del controlador `LoginController`
- Creada la ruta `gracias`
- Creada la funcion `gracias` dentro del controlador `PaginasController`
- Creada la tabla `usuario_sesion`
- Implementado el modelo `UsuarioSesion` para interactuar con la base de datos
- Implementadas sesiones persistentes mediante cookies

### 🔧 Cambiado
- En la ruta `/tienda`:
    - Se agrego una barra de busqueda para buscar productos por nombre/categoria
    - Se agrego la opcion de elegir el orden en el que aparecen los productos
    - Se agrego paginacion a la lista de productos
    - Se agrego una validación para limitar a 99 la cantidad de cada producto individual en el pedido
- Modificado el archivo `database.sql` para mantener coherencia con la nueva estructura

### 🐛 Corregido
- Arreglado error en la funcion `guardarNuevosDatos` del controlador `APIDatosUsuario`
- Arreglado error al intentar registrar una cuenta nueva

## [0.9.0] - 2025-09-24
### ➕ Agregado
- Creada funcion `cargarIconos` dentro del archivo `tienda.js`
- Agregados primeros ajustes visuales en la zona publicas y privadas de la tienda
- Creada funcion `is_banned` dentro del archivo `funciones.php` para verificar el baneo del usuario

### 🔧 Cambiado
- Se cambiaron los nombres de varios archivos .svg
- Se quito la opcion `Card` del boton de Paypal
- En la ruta `/registro`:
    - Se agrego el campo `telefono` al formulario
    - Se cambiaron los mensajes de alerta
- Reescrito `usuarioDatos.js` para manejar elementos dinámicos y validación vía endpoint
- Agregadas varias funciones al controlador `APIDatosUsuario`:
    - `validarDatos`
    - `validarFormularioUsuario`
    - `guardarNuevosDatos`
- En el modelo `Usuario`:
    - Se cambio el nombre de la funcion `setear_cambio_contraseña` a `setearCambioContraseña`
    - Se cambio el nombre de la funcion `validar_cambio_contraseña` a `validarCambioContraseña` y su funcionalidad
    - Se agregaron las funciones:
        - `validarCambioDatos`
        - `validarCambioCorreo`
        - `validarCambioMisDatos`
- Se actualizo la funcion `usuario` del modelo `UsuariosController` 

### 📅 Pendiente
- Detalles finales de la tienda

## [0.8.0] - 2025-08-27
### ➕ Agregado
- Creada funcion `rutaActual` en el archivo `funciones.php`
- Creado el archivo `formulario.js` para mejoras visuales de los formularios en la pagina
- Agregados primeros ajustes visuales en la zona de administración

### 📅 Pendiente
- Verificacion de baneo del usuario al loguear
- Mejoras del diseño visual en la zona publica
...
- Añadida funcionalidad a la ruta `admin/pedidos`
- Creadas las rutas:
...
- Añadida funcion `getPaisesAceptados` en el modelo `Direccion` 
- Actualizada función `recuperarCarrito` del controlador `APICarrito` para crear un carrito nuevo en caso de que el usuario no tenga uno asignado
...
- Agregadas varias funciones al modelo `ActiveRecord`:
    ...
    - `thisByDate`
    - `this` 
- Cambios en la ruta `/usuario` para añadir la columna `telefono` al mostrar y editar los datos del usuario
...
- Agregadas varias funciones al modelo `Producto`:
    ...
    - `masVendido`
    - `masVendidoAll`
- Se agregó un índice único (`email_UNIQUE`) sobre la columna `email` de la tabla `usuarios`.
- Agregadas varias columnas a la tabla `usuarios`:
    - `fecha_creacion`
    - `baneo`
- Actualizado modelo `usuario` para mantener coherencia con su nueva estructura 
- Se modificó el archivo `database.sql` para mantener coherencia con la nueva estructura
